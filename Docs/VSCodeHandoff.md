...
- iOS Player Settings updated 15 September 2026: `targetDevice = iPhoneAndiPad` and `iOSTargetOSVersionString = 26.0`. Unity live verification returned those exact values; recompile status was `completed` with `compilationFailed: false`.
...
- Boat cutscene test wiring on 15 September 2026: `BoatArrivalController` is now attached to `PF_Boat_Parent`; `BoatInteractable` is attached to the existing `PF_Boat_Parent/Boat/BoatFloor` collider. They use `BoatApproachStart`, `BoatDockedEnd`, `ArrivalPlayerStandPoint`, and `BoatDisembarkPoint`, and the controller uses the existing `GameManager`'s `JourneyProgressService`. The disembark path now records `arrival_complete` and the `arrival_landing` checkpoint. A live Play Mode observation reached `Approaching` with player state `OnBoat`, `CanDisembark = false`, and interaction disabled. Time still did not advance in this Editor session, so docking/disembark completion remains a focused-Game-view test.
- Continue menu on 15 September 2026: `LV_MainMenu/UI_MainMenu/CanvasMainMenu/MainMenuPanel/MainMenuButtonsPanel/ButtonContinue` was cloned from `ButtonStartGame`, preserving its uGUI layout, visual style, EventTrigger, TMP font, and button transition. `MainMenuController` owns the cloned button; it hides it without a checkpoint, binds it to `ContinueJourney()` when a checkpoint exists, and `StartGame()` now clears progress for New Journey.
...
- A pre-whitebox reference copy now exists at `Assets/Scenes/Archive/LV_TestScene_PreWhitebox.unity`. Do not edit it; it is a reference snapshot of the playable scene before the route markers were added.
- Whitebox pass begun on 15 September 2026: `LV_TestScene` has a `WhiteboxRoute` root with nine non-colliding, coloured primitives. Its groups mark arrival/movement tutorial, optional CV-file and seat reflection near the campfire, the matches/campfire return, the fire-rain path, and the lighthouse entry/upper-stairs goal. It does not add gameplay blockers or replace environment art.
- Interaction audit: [InteractionAudit.md](InteractionAudit.md) records the current code against the intended route. Boat arrival is the only relevant interaction already tied to save-owned progress. `CollectibleItem`, `FireStarter`, `Door`, and `SeatInteractable` mainly own local runtime state; `FireStarter.HasMatches` is still static and must be replaced before it is used for narrative progress. No readable-file or ending-trigger interaction exists yet.
- Swimming audit: the PlayerController feature is disabled and only switches from a global water Y-plane; `Water_01` has no trigger/collider. It is not part of the first land route. Any future swimming work starts with local water volumes and an explicit recovery/shore design, then mobile device validation.
- Verification after the whitebox pass: Unity recompilation returned `completed` with `compilationFailed: false`; `JourneyProgressStoreTests` passed 3/3 and `ArrivalSequenceStateMachineTests` passed 3/3 in EditMode. `git diff --check` reports pre-existing-style trailing whitespace emitted in `LV_TestScene.unity` by Unity serialization; no scene YAML was hand-edited.
- Recommended next action: in a visibly advancing Game view, route-test the boat through docking/disembark and the new markers, then implement the persistent matches-to-campfire vertical slice (including restored fire state and an event-channel transition to the fire-rain path).
