...
- Recommended next action: run the menu-to-`LV_TestScene` route in a normally advancing Game view, then compare it with a direct `LV_TestScene` start. Verify boat arrival/disembark, movement, interactions, pause, return to menu, and replay before changing configuration.
