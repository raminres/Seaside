...
- Recommended next action: in a visibly advancing Game view, run the wired boat route through dock and disembark, verify the movement tutorial, then restart and confirm Continue skips the approach from `arrival_landing`. Follow with pause/resume, return-to-menu, and direct-entry regression.
